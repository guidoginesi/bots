...
- **Video**: la grabación es más pesada (CPU/RAM) y más lenta que las fotos. Si el `.webm` sale vacío o la función muere, subir `memory` en `vercel.json` (ojo con el tope del plan — por eso está en 1024) y/o acortar el recorrido. Vigilar `maxDuration: 300`.
